...
def draw_curve_points(points):
    ...
    while True:
        for n in range(0, 4):
            for i in range(0, 100, 2):
                t = i / 100
                x = ((-t**3 + 2*t**2 - t)*points[n % 4][0] + (3*t**3 - 5*t**2 + 2)*points[(n + 1) % 4][0] + (-3*t**3 + 4*t**2 + t)*points[(n + 2) % 4][0] + (t**3 - t**2)*points[(n + 3) % 4][0])/2
                y = ((-t**3 + 2*t**2 - t)*points[n % 4][1] + (3*t**3 - 5*t**2 + 2)*points[(n + 1) % 4][1] + (-3*t**3 + 4*t**2 + t)*points[(n + 2) % 4][1] + (t**3 - t**2)*points[(n + 3) % 4][1])/2
                draw_point((x, y))
